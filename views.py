...
def show_residency(request,page):
    ...
    print >>sys.stderr,"show_residency() slug: %r", slug
...
def show_organization(request,page,id=None):
    ...
    org=Organization.objects.filter(id=1)[0]
...
def return_formentry(slug):
    ...
    print >>sys.stderr,"return_formentry(): slug:%r" % slug
    single_form = Form.objects.get(slug=slug)
    print >>sys.stderr,"found single_form", single_form
...
def list(request, slug=None):
    ...
    for v in rv['entrylist']:
        ...
        for w in v:
            print >>sys.stderr, "\t", w
    return render_to_response('list.html', {'slug':slug,'rv':rv, 'entrylist': rv['entrylist']})
...
def rawlist(request, slug=None):
    ...
    return render_to_response('rawlist.html', {'slug':slug, 'rv':rv, 'entrylist': rv['entrydictlist']})
...
def showorganization(request, organization=None):
    ...
    return render_to_response('show_organization.html', {'rv':rv})
